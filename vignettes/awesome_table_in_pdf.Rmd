...
the step every time.* **In this tutorial, I’ll still put format="latex" in the function in
...
In `kableExtra` 0.3.0 or above, a new option `repeat_header` was introduced into `kable_styling`. It will add header rows to longtables spanning multiple pages. For table captions on following pages, it will append *"continued"* to the caption to differentiate. If you need texts other than *"(continued)"* (for example, other languages), you can specify it using `kable_styling(..., repeat_header_text = "xxx")`. In case you would like to pass on formatting to this option, you will have to escape it twice. Once with quotes and once for the backslash: `kable_styling(..., repeat_header_text = "\\emph{xxx}")`. If you want to completely replace the table caption instead of appending, you can specify it in the option `repeat_header_method`.
```{r}
